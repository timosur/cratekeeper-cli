...
def enrich_tracks_with_artist_genres(sp: spotipy.Spotify, tracks: list[Track]) -> None:
    """Fetch artist genres and assign them to each track in-place."""
...
def create_event_playlists(
    sp: spotipy.Spotify,
    plan,  # EventPlan — avoid circular import
    event: str,
    date: str,
    progress_callback=None,
) -> None:
    """Create per-bucket Spotify playlists for an event and store IDs in the plan."""
    ...
    for bucket_name, bucket_tracks in buckets.items():
        ...
        if progress_callback:
            progress_callback(playlist_name, len(track_ids))


def build_master_playlists(
    sp: spotipy.Spotify,
    plan,  # Plan
    progress_callback=None,
) -> tuple[int, int]:
    """Add classified tracks to cross-event [DJ] master playlists, deduplicating.

    Returns (total_added, total_dupes).
    """
    buckets = plan.bucket_summary()
    ...
    for bucket_name, bucket_tracks in buckets.items():
        ...
        if progress_callback:
            progress_callback(master_name, len(new_ids), dupes)

    return total_added, total_dupes
