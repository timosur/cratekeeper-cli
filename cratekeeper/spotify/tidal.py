...
def sync_plan_to_tidal(
    session: tidalapi.Session,
    plan,  # EventPlan
    progress_callback=None,
) -> tuple[int, int]:
    """Create per-bucket Tidal playlists from a classified plan and store IDs.

    Returns (total_added, total_failed).
    """
    ...
    for bucket_name, bucket_tracks in buckets.items():
        ...
        if not isrcs:
            if progress_callback:
                progress_callback(bucket_name, 0, 0, skipped=True)
            continue
        ...
        if progress_callback:
            progress_callback(playlist_name, len(added), len(failed), skipped=False)
