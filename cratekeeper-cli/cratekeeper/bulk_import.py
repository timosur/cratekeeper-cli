...
from cratekeeper.local_scanner import _get_conn
...
def import_tracks(source_path: Path, db_url: str | None = None) -> list[Track]:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT path, title, artist, album, isrc, year, duration_ms
                FROM tracks
