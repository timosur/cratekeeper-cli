...
### Step 6b: Purchase & Integrate Missing Tracks (optional)

If tracks are missing from the local library, the user can buy them and integrate them into the pipeline. Share `_missing.txt` or `.missing-tidal.txt` with the user and walk them through this process:

1. **Review missing tracks** — open `.missing.txt` (human-readable) or `.missing-tidal.txt` (direct Tidal links) generated by the match step.
2. **Purchase tracks** — buy from Tidal, Beatport, Bandcamp, or any other store. Download the files.
3. **Drop purchased files into a staging directory** — e.g., `~/Downloads/purchased/` or any folder. Filenames should roughly match the track titles (most stores do this by default).
4. **Tag the purchased files** with metadata from the plan:

```bash
crate tag-untagged data/<slug>.classified.json ~/Downloads/purchased/
```

This matches downloaded files to unmatched tracks by normalizing filenames against track titles, then writes basic metadata (title, artist, album, year, ISRC) into the audio tags.

5. **Move tagged files to the NAS library** — manually copy/move the tagged files from the staging directory into `/Volumes/Music` (or wherever the library lives), following your existing folder structure.
6. **Re-scan the library** to pick up the new files:

```bash
crate scan /Volumes/Music
```

7. **Re-match** to verify the purchased tracks are now found:

```bash
crate match data/<slug>.classified.json --tidal-urls
```

The previously missing tracks should now show as matched. Any remaining unmatched tracks may need manual intervention (wrong filename, different version, etc.).
