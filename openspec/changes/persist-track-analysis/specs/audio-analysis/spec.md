...
The system SHALL analyze each matched audio file to extract BPM, musical key, energy, danceability, loudness, and mood descriptors using essentia and pre-trained TensorFlow models, running natively via pip-installed essentia-tensorflow on macOS Apple Silicon or Linux x86_64. Before running analysis, the system checks a persistent cache keyed by file content hash.

#### Scenario: Full audio analysis
- **WHEN** the DJ runs `crate analyze-mood` on a plan with matched tracks
- **THEN** the system computes the SHA256 content hash of each matched track's audio file
- **AND** checks the analysis cache for existing results
- **AND** for cache misses, extracts BPM, key, energy, danceability, loudness, and mood predictions using essentia
- **AND** persists results in both the plan JSON and the analysis cache
...
- **WHEN** audio analysis completes (from cache or fresh extraction)
