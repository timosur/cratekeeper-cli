...
**Reason**: essentia-tensorflow now ships native ARM64 wheels for macOS (since v2.1b6.dev1389) and continues to support Linux x86_64 via pip. Docker/QEMU emulation is no longer needed for audio analysis.

**Migration**: Run `pip install essentia-tensorflow` directly. Remove the `crate` service from `docker-compose.yml`. The `db` service (PostgreSQL) remains unchanged.
