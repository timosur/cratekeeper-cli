...
The system SHALL present an interactive review of each candidate track (those with both `local_path` and `bucket`) and record an approve, reject, or skip decision that persists in the event-plan JSON.
...
- **THEN** the track's `library_approval` is set to `"approved"` and persisted in the plan JSON

#### Scenario: Reject a track
- **WHEN** the DJ presses `r` during review
- **THEN** the track's `library_approval` is set to `"rejected"` and persisted in the plan JSON
